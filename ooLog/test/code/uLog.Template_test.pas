...
begin
  CheckEquals(Format('%.2d', [WeekOfTheYear(Date)]), TLogTemplate.New('{WeekYear}',
      TTextMatchInsensitive.New).Build(EmptyStr, llInfo));
end;
...
begin
  CheckEquals(Copy(TLogTemplate.New('{Time}', TTextMatchInsensitive.New).Build(EmptyStr, llInfo), 1, 11),
    Copy(FormatDateTime('HH:NN:SS.ZZZ', Time), 1, 11));
end;

procedure TLogTemplateTest.TestDateTime;
begin
  CheckEquals(Copy(TLogTemplate.New('{DateTime}', TTextMatchInsensitive.New).Build(EmptyStr, llInfo), 1, 22),
...
begin
  CheckEquals(Copy(FormatDateTime('ZZZ', Time), 1, 2),
    Copy(TLogTemplate.New('{MiliSecond}', TTextMatchInsensitive.New).Build(EmptyStr, llInfo), 1,
      2));
end;

procedure TLogTemplateTest.TestApp;
begin
  CheckEquals(Application.Title, TLogTemplate.New('{App}', TTextMatchInsensitive.New).Build(EmptyStr, llInfo));
end;
...
begin
  CheckEquals('DEBUG', TLogTemplate.New('{LogLevel}', TTextMatchInsensitive.New).Build(EmptyStr, llDebug));
  CheckEquals('INFO', TLogTemplate.New('{LogLevel}', TTextMatchInsensitive.New).Build(EmptyStr, llInfo));
  CheckEquals('WARNING', TLogTemplate.New('{LogLevel}', TTextMatchInsensitive.New).Build(EmptyStr, llWarning));
  CheckEquals('ERROR', TLogTemplate.New('{LogLevel}', TTextMatchInsensitive.New).Build(EmptyStr, llError));
end;
...
begin
  CheckEquals('First line{#13}{#10}Second line{#13}{#10}Third line', TLogTemplate.New('{Text}',
      TTextMatchInsensitive.New).Build('First line' + sLineBreak + 'Second line' + sLineBreak + 'Third line', llInfo));
end;
...
begin
  ...
  CheckEquals('One text{TAB}two text', LogTemplate.Build('One text' + Chr(9) + 'two text', llInfo));
end;
