...
begin
  ...
  if chkLevelLog.Checked then
    Include(LevelFilter, llDebug);
  if chkLevelInfo.Checked then
    Include(LevelFilter, llInfo);
  if chkLevelWarning.Checked then
    Include(LevelFilter, llWarning);
  if chkLevelError.Checked then
    Include(LevelFilter, llError);
  _Logger.ChangeFilter(LevelFilter);
end;

procedure TMainForm.FilterSetToControls;
begin
  chkLevelLog.Checked := llDebug in _Logger.Filter;
  chkLevelInfo.Checked := llInfo in _Logger.Filter;
  chkLevelWarning.Checked := llWarning in _Logger.Filter;
  chkLevelError.Checked := llError in _Logger.Filter;
end;
